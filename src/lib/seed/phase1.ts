import phase1SeedData from '../../../supabase/seeds/phase1.seed.json'
...
export type Phase1SeedData = {
  categories: Phase1Category[]
  sites: Phase1Site[]
}

export const PHASE1_SEED_DATA = phase1SeedData as Phase1SeedData

export function validatePhase1SeedData(data: Phase1SeedData): void {
  ...
  if (uniqueUrls.size !== data.sites.length) {
    throw new Error('Duplicate site URLs found in phase 1 seed data')
  }

  const uniqueHostnames = new Set(data.sites.map((site) => new URL(site.url).hostname.toLowerCase()))
  if (uniqueHostnames.size !== data.sites.length) {
    throw new Error('Duplicate site domains found in phase 1 seed data')
  }
...
export async function seedPhase1Data(client: any, data = PHASE1_SEED_DATA): Promise<void> {
  validatePhase1SeedData(data)
